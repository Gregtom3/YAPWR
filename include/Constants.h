...
static const double INVERSION_LAMBDA = 0.02;
...
/**
 * Return the first (idx=0) or second (idx=1) hadron’s LaTeX label
 * from a pair string like "piplus_pi0".
 * Falls back to the raw token if not found.
 */
inline std::string hadronLatexFromPair(const std::string& pairName, int idx) {
    auto pos = pairName.find('_');
    std::string first = pairName.substr(0, pos);
    std::string second = (pos == std::string::npos) ? "" : pairName.substr(pos + 1);
    auto const& map = hadronNameLatex();
    const auto& key = (idx == 0 ? first : second);
...
inline std::string firstHadronLatex(const std::string& pairName) {
    return hadronLatexFromPair(pairName, 0);
}
inline std::string secondHadronLatex(const std::string& pairName) {
    return hadronLatexFromPair(pairName, 1);
}
