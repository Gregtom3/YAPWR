...
{
  return (t>0)
        ? std::sqrt( std::pow(ds/t,2) + std::pow(s*dt/(t*t),2) )
        : 0.0;
}
...
//  Helper for one grid --------------------------------------------------------
//==============================================================================
static void doGrid(const std::vector<double>& phi_h,
                   ...
                   std::vector<BinInfo>&   binTable)        // N elements
{
  ...
  // prepare canvas
  TCanvas c(Form("c_%s_%dx%d",pairName,N,M),
            Form("Purity %s %dx%d",pairName,N,M),
            M*500,N*500);
  c.Divide(M,N,0,0);
  gStyle->SetOptStat(0);
      ...
      for (int j=0;j<M;++j) {
          ...
          for (auto&p:slice)
              if (p.first>=info.rEdges[j] && p.first<info.rEdges[j+1])
                  h->Fill(p.second);
          h->Sumw2();  double area=h->Integral();  if(area>0) h->Scale(1.0/area);
          ...
          double sig_err = std::sqrt(
              std::pow(std::sqrt(2*M_PI)*sig->GetParameter(2)*sig->GetParError(0),2)+
          ...
          // -------------------------------- draw -----------------------------
          int pad=i*M+j+1; c.cd(pad);
          h->SetLineColor(kBlack); h->SetLineWidth(2);
          fit->SetLineColor(kRed); fit->SetLineWidth(2);
          sig->SetLineColor(kBlue);sig->SetLineWidth(2);
          h->Draw("hist"); fit->Draw("same"); sig->Draw("same");

          TLatex L; L.SetNDC(); L.SetTextSize(0.05);
          L.DrawLatex(0.5,0.9,
              Form("%.3f<#phi_{h}<%.3f",hlo,hhi));
          L.DrawLatex(0.5,0.85,
              Form("%.3f<#phi_{R}<%.3f",info.rEdges[j],info.rEdges[j+1]));
          L.DrawLatex(0.5,0.8,
              Form("#mu=%.3f#pm%.3f",fit->GetParameter(1),fit->GetParError(1)));
          L.DrawLatex(0.5,0.75,
              Form("#sigma=%.3f#pm%.3f",fit->GetParameter(2),fit->GetParError(2)));
          L.DrawLatex(0.5,0.7,
              Form("#chi^{2}/ndf=%.2f", (fit->GetNDF()?fit->GetChisquare()/fit->GetNDF():0) ));
          L.SetTextColor(kBlue);
          L.DrawLatex(0.5,0.65,
              Form("purity=%.3f#pm%.3f",purity,purity_err));
          L.SetTextColor(kBlack);
          L.DrawLatex(0.5,0.6,
              Form("Events = %.0f",h->GetEntries()));
      } // j loop
                   ...
                   const char* outputDir)
{
  ...
  if(!f||f->IsZombie()){std::cerr<<"Cannot open "<<inputPath<<"\n";return;}
  ...
  t->SetBranchStatus("phi_h",1);  t->SetBranchAddress("phi_h",&ph);
  t->SetBranchStatus("phi_R1",1); t->SetBranchAddress("phi_R1",&pr);
  t->SetBranchStatus("M2",1);     t->SetBranchAddress("M2",&m2);

  const Long64_t nEnt = t->GetEntries();
  std::vector<double> vph,vpr,vm2;
  vph.reserve(nEnt);vpr.reserve(nEnt);vm2.reserve(nEnt);
