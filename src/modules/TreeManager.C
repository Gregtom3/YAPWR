...
// ------------------------------------------------------------------
void loadEntryList(TTree* tree, const char* yamlPath, bool useTrueVariable = false) {
    if (!tree) {
        std::cerr << "[loadEntryList] null TTree pointer\n";
        return;
    }
    ...
    if (pairKey.empty()) {
        std::cerr << "[loadEntryList] could not infer pair name from " << searchStr << '\n';
        return;
    }
    ...
    std::cout << "[loadEntryList] " << pairKey << (useTrueVariable ? " (true vars)" : "") << " → \"" << cutExpr << "\"  ("
              << elist->GetN() << " / " << tree->GetEntries() << " entries kept)\n";
}
} // namespace util
