...
static std::vector<TLeaf*> numericLeaves(TTree* t) {
    ...
    for (int i = 0; i < L->GetEntries(); ++i) {
        ...
        if (tn == "Double_t" || tn == "Float_t" ||
            tn == "Int_t"   || tn == "UInt_t"  || tn == "Long64_t")
            v.push_back(leaf);
    }
    return v;
}
...
// ------------------------------------------------------------------
static void writeCsvRDF(TTree* t,
                        const std::vector<TLeaf*>& leaves,
                        ...
                        const char*                cutYamlPath)
{
    ...
    for (auto& m : leafMeans) csv << *m << ',';
    csv << *mP00 << ',' << *mP10 << ',' << *mP11 << ',' << *mP1m1 << ','
        << *mP20 << ',' << *mP21 << ',' << *mP2m1 << ',' << *mP22 << ','
        << *mP2m2 << '\n';
    csv.close();

    std::cout << "  wrote " << outfile << "  (" << n << " events, "
              << sw.RealTime() << " s wall‑time, "
              << sw.CpuTime()  << " s CPU)\n";
}
                   ...
                   const char* outDir)
{
    ...
    if (p == "piplus_pi0" || p == "piminus_pi0") {
        writeCsvRDF(t, leaves,
                    "M2>0.106 && M2<0.166",
                    std::string(outDir)+"/signal.csv",
                    cutYamlPath);

        writeCsvRDF(t, leaves,
                    "M2>0.2 && M2<0.4",
                    std::string(outDir)+"/background.csv",
                    cutYamlPath);
    }
    f.Close();
}
