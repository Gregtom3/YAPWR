...
namespace {
    ...
    const bool registered = [](){
        ModuleProcessorFactory::instance()
            .registerProcessor("baryonContamination", make);
        return true;
    }();
}
...
Result BaryonContaminationProcessor::process(const std::string& moduleOutDir,
                                             const Config& cfg)
{
...
BaryonContaminationProcessor::ContaminationData
BaryonContaminationProcessor::loadData(const std::string& moduleOutDir) const
{
